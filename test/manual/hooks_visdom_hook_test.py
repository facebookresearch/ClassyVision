...
class TestVisdomHook(HookTestBase):
    ...
    @mock.patch("classy_vision.hooks.visdom_hook.is_leader")
    @mock.patch("classy_vision.hooks.visdom_hook.Visdom", autospec=True)
    def test_visdom(
        self, mock_visdom_cls: mock.MagicMock, mock_is_leader: mock.MagicMock
    ) -> None:
        """
        ...
        for master, visdom_conn in product([False, True], [False, True]):
            mock_is_leader.return_value = master
